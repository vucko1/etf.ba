...
var Candy = function(self, $) {
    ...
    self.init = function(service, options) {
        ...
        options.viewClass.init($("#haber"), options.view);
...
Candy.Core = function(self, Strophe, $) {
    ...
    _rooms = {}, /** PrivateVariable: _anonymousConnection
		 ...
		 */
    _anonymousConnection = true, /** PrivateVariable: _status
...
Candy.Util = function(self, $) {
    ...
    self.Parser = {
        ...
        emoticons: [ {
            ...
            plain: ":shit:",
            regex: /((\s):shit:|:shit:(\s|$))/gm,
            image: "shit.gif"
        }, {
            plain: ":hrkljus:",
            regex: /((\s):hrkljus:|:hrkljus:(\s|$))/gm,
            image: "hrkljus.gif"
        }, {
            plain: ":cookie:",
            regex: /((\s):cookie:|:cookie:(\s|$))/gm,
            image: "cookie.gif"
        }, {
            plain: ":crab:",
            regex: /((\s):crab:|:crab:(\s|$))/gm,
            image: "crab.gif"
        }, {
            plain: ":killx:",
            regex: /((\s):killx:|:killx:(\s|$))/gm,
            image: "kill.gif"
        } ],
        /** Function: emotify
            ...
            Message: function(msg) {
                ...
                if (msg.children("subject").length > 0) {
                    ...
                    if (msg.attr("type") === "chat" || msg.attr("type") === "normal") {
                        ...
                        isNoConferenceRoomJid = !Candy.Core.getRoom(bareRoomJid), name = isNoConferenceRoomJid ? Strophe.getNodeFromJid(roomJid) : Strophe.getResourceFromJid(roomJid);
                        message = {
                            name: name,
...
Candy.View.Observer = function(self, $) {
    ...
    self.Chat = {
		...
		ShowMsg: function(msg, color, hide) {
			var authtag=$('<div onclick="$(this).fadeOut();" style="background-color:'+color+'" class="haber-error-message bold"><span class="haber-error-time normal">SERVER:</span> '+msg+'</div>');
				$('#DiscussionPanel').prepend(authtag);
				if(hide===1)
					authtag.delay(3000).fadeOut(1000);
        ...
        Connection: function(event, args) {
            ...
            switch (args.status) {
              case Strophe.Status.CONNECTING:
              case Strophe.Status.AUTHENTICATING:
                Candy.View.Observer.Chat.ShowMsg($.i18n._("statusConnecting"),"#D0D102",1);
              ...
              case Strophe.Status.CONNECTED:
                if (_showConnectedMessageModal === true) {
					...
					Candy.View.Observer.Chat.ShowMsg($.i18n._("statusConnected"),"#A0D36E",1);
              ...
              case Strophe.Status.DISCONNECTING:
				Candy.View.Observer.Chat.ShowMsg($.i18n._("statusDisconnecting"),"#F18D05",0);
              ...
              case Strophe.Status.DISCONNECTED:
			   ...
			   Candy.View.Observer.Chat.ShowMsg($.i18n._("statusDisconnected"),"#E54028",0);
//				Candy.View.Pane.Chat.Modal.show($.i18n._("statusDisconnecting"), false, true);
              ...
              case Strophe.Status.AUTHFAIL:
               // Candy.View.Pane.Chat.Modal.showLoginForm($.i18n._("statusAuthfail"));
				Candy.View.Observer.Chat.ShowMsg($.i18n._("statusAuthfail"),"#E54028",0);
                break;

              default:
				Candy.View.Observer.Chat.ShowMsg($.i18n._("status", args.status),"#00A1CB", 0);
...
Candy.View.Pane = function(self, $) {
            ...
            init: function() {
                $("#emoticons-icon").click(function(e) {
				 ...
				 $("#archive-icon").click(function(e) {
                    window.open("http://etf.ba/haberx/archive.php",'_blank');
                    e.stopPropagation();
                });
				$("#token-icon").click(function(e) {
					alert("Token(password) za mobitel je: "+token);
					e.stopPropagation();
				});
    ...
    self.Room = {
        ...
        appendToMessagePane: function(roomJid, html) {
            self.Room.getPane(roomJid, ".message-pane").prepend(html);
    ...
    self.Message = {
        ...
        show: function(roomJid, name, message, timestamp) {
            ...
            self.Room.appendToMessagePane(roomJid, html);
            var elem = self.Room.getPane(roomJid, ".message-pane").children().first();
...
Candy.View.Template = function(self) {
    ...
    self.Chat = {
        ...
        toolbar: '<ul id="chat-toolbar">' + '<li id="token-icon" title="token za mobitel"></li><li id="archive-icon" title="arhiva habera"></li><li id="emoticons-icon" title="smajliji" data-tooltip="{{tooltipEmoticons}}"></li>' + '<li id="chat-sound-control" class="checked" data-tooltip="{{tooltipSound}}">{{> soundcontrol}}</li>' + '<li class="checked" title="prikazuj statuse join/left" id="chat-statusmessage-control" data-tooltip="{{tooltipStatusmessage}}">' + '</li><li class="context" data-tooltip="{{tooltipAdministration}}"></li>' + '<li class="usercount" onclick="$(\'.room-pane\').filter(function() { return $(this).css(\'display\') == \'block\'; }).find(\'.roster-pane\').toggle(\'slow\')" data-tooltip="{{tooltipUsercount}}">' + '<span id="chat-usercount" ></span></li></ul>',
    ...
    self.Room = {
        ...
        form: '<div class="message-form-wrapper">' + '<form method="post" class="message-form">' + '<input name="message" class="field" type="text" aria-label="Message Form Text Field" placeholder="Haber poruka ..." autocomplete="off" maxlength="300" />' + '<input type="submit" class="submita" name="submit" value="{{_messageSubmit}}" /></form></div>'
    };
    self.Roster = {
        pane: '<div class="roster-pane" style="display:none;"></div>',
...
Candy.View.Translation = {
    en: {
       		'status': 'Status: %s',
		'statusConnecting': 'Spajam se na haber kutiju...',
		'statusConnected' : 'Spojen na haber kutiju',
		'statusDisconnecting': 'Diskonektujem se sa haber kutije...',
		'statusDisconnected' : 'Konekcija prekinuta prema haber kutiji',
		'statusAuthfail': 'Problem sa autorizacijom na haber kutiji',

		'roomSubject'  : 'Naslov:',
		'messageSubmit': 'Pošalji',

		'labelUsername': 'Username:',
		'labelNickname': 'Nadimak:',
		'labelPassword': 'Password:',
		'loginSubmit'  : 'Login',
		'loginInvalid'  : 'Pogresan JID',

		'reason'				: 'Razlog:',
		'subject'				: 'Naslov:',
		'reasonWas'				: 'Razlog: %s.',
		'kickActionLabel'		: 'Kick',
		'youHaveBeenKickedBy'   : 'Kicknuti ste sa habera od strane %1$s',
		'youHaveBeenKicked'     : 'Kicknuti ste sa habera od strane %s',
		'banActionLabel'		: 'Ban',
		'youHaveBeenBannedBy'   : 'Izbaceni ste sa habera od strane %1$s by %2$s',
		'youHaveBeenBanned'     : 'Izbaceni ste sa habera od strane %s',

		'privateActionLabel' : 'Privatni chat',
		'ignoreActionLabel'  : 'Ignorisi',
		'unignoreActionLabel' : 'Prestani ignorisati',

		'setSubjectActionLabel': 'Promijeni naslov',

		'administratorMessageSubject' : 'Administrator',

		'userJoinedRoom'           : '%s je započeo haber.',
		'userLeftRoom'             : '%s ima pametnijeg posla.',
		'userHasBeenKickedFromRoom': '%s je kicknut.',
		'userHasBeenBannedFromRoom': '%s je banovan.',
		'userChangedNick': '%1$s je promijenio nadimak u %2$s.',

		'presenceUnknownWarningSubject': 'Obavještenje:',
		'presenceUnknownWarning'       : 'Korisnik je možda offline.',

		'dateFormat': 'dd.mm.yyyy',
		'timeFormat': 'HH:MM:ss',

		'tooltipRole'			: 'Moderator',
		'tooltipIgnored'		: 'Ignoriši ovog korisnika',
		'tooltipEmoticons'		: 'Smajliji',
		'tooltipSound'			: 'Zvuk za privatnu poruku',
		'tooltipAutoscroll'		: 'Autoscroll',
		'tooltipStatusmessage'	: 'Prikaži status poruku',
		'tooltipAdministration'	: 'Administratori habera',
		'tooltipUsercount'		: 'Korisnici',

		'enterRoomPassword' : 'Soba "%s" je zasticnea passwordom.',
		'enterRoomPasswordSubmit' : 'UDi u sobu',
		'passwordEnteredInvalid' : 'Pogresan password za sobu "%s".',

		'nicknameConflict': 'Korisnicko ime se vec koristi izaberite drugo.',

		'errorMembersOnly': 'Ne mozes pristupiti sobi "%s": Nemas pravo.',
		'errorMaxOccupantsReached': 'Ne moezs pristupiti sobi "%s": PRevise prisutnih.',
		'errorAutojoinMissing': 'Nema autojoin.',

		'antiSpamMessage' : 'Nemoj spamati, banovan si kratko.'
    },
    de: {
        status: "Status: %s",
